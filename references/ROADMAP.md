...
## Slice 23 — BookBank Reader (shared reading room, no annotations)

Integrate the public **BookBank** library (`sunprema.github.io/books`) so a workspace can add an AI-authored HTML book to its library and read it **together** inside the StudySync reader shell. Books are multi-page self-contained HTML (one file per concept, listed in the book's `book.json`), not PDFs — so this slice introduces a second `Resource` kind and a branch in the reader.

This is the **integrated, read-only** cut: the collaborative value comes from **concept-level milestones + rubber stamps** (progress), **chat**, and **presence** — all annotation-independent features that already exist. Annotations, threads, and the AI assistant are **explicitly out of scope** here.

**Design decisions (read before touching code):**

- **concept = page.** Each BookBank concept is treated as a "page": `Resource.page_count` = concept count, and the reader's `:focal_page` is the focal concept index (1-based). This lets the existing chapter rail, milestone, rubber-stamp, sprint, and presence machinery work with no changes to their page-oriented shape.
- **No same-origin proxy, no bridge script, no text anchoring.** Because we render read-only, the book is shown in a **sandboxed iframe pointed directly at its public GitHub Pages URL** (`sandbox="allow-scripts"`, no `allow-same-origin`). We never read selection or inject into the book's DOM, so the proxy + bridge + TextQuoteSelector subsystem the annotations path would need is deliberately absent. If HTML annotations are wanted later, that's a **separate future slice** (proxy the concept HTML same-origin + inject a selection/marker bridge + `anchor :map` on `Annotation`) — do not smuggle any of it in here.
- **Progress comes from stamps, not annotations.** `Resource.progress_percent` / `time_spent_seconds` are derived from annotations and will read empty for bookbank resources. A bookbank resource's progress is "concepts stamped / total milestones," so milestones + rubber stamps are the progress spine, not a bolt-on.
- **PDF-only features are hidden for bookbank**, not made to half-work: the Concept Map Board (`+ Board`, renders PDF page thumbnails), the annotation margin column, reading-journal export, and the AI "Ask AI" path do not apply. The reader branch must hide their entry points when `kind == :bookbank`.

- [ ] **23.1** Extend `Studysync.Library.Resource`: add `kind :atom` `one_of [:pdf, :bookbank]` default `:pdf` (existing rows stay `:pdf`); add `external_id :string` (e.g. `"git-internals"`), `source_url :string` (e.g. `https://sunprema.github.io/books/git-internals/`), and `concepts :map` (snapshot of `[%{id, title, file, index}]` captured at import so ordering is stable without re-fetching `book.json`). Make `file_path` nullable (bookbank stores nothing locally). Migration via `mix ash.codegen add_bookbank_resource_kind`.
- [ ] **23.2** `Studysync.Bookbank.Source` module — thin Req client (mirrors `library/resource/changes/fetch_url.ex`) with a **host allowlist** (`sunprema.github.io` only). `fetch_catalog/0` gets `/catalog.json`; `fetch_book/1` gets `/books/:id/book.json`. ETS-cache both (mimic `Studysync.Chat.Buffer`'s named-table pattern) since published content is effectively immutable — key the book cache off the book's `offline.json` `version` when available. Never fetch a non-allowlisted host.
- [ ] **23.3** Ash action `create :import_bookbank` on `Resource` — args `workspace_id`, `url` (the book directory URL, e.g. `https://sunprema.github.io/books/books/protobuf/`). This URL is the canonical input so both entry points in 23.4 funnel through one path. A `Changes.ImportBookbank` change validates the URL's host against the allowlist (`sunprema.github.io`), derives `external_id` from the path, calls `Bookbank.Source.fetch_book/1` (which reads `<url>book.json`), and writes `kind: :bookbank`, `title`, `source_url: url`, `concepts` (indexed), `page_count: length(concepts)`, `file_path: nil`. A non-allowlisted host or an unreachable/invalid `book.json` returns a validation error, not a crash. Policy: only workspace members may import into their workspace (reuse the existing resource-create policy). Domain fn `import_bookbank/2`.
- [ ] **23.4** Two entry points, both calling `import_bookbank`, both in the workspace library header:
  - **Catalog browser** — `StudysyncWeb.BookbankLive.Browse` (or a modal): lists the catalog (cover, title, summary, concept count) from `Bookbank.Source.fetch_catalog/0`; each row's "Add to workspace" button submits that book's directory URL. Themed per Direction 01 (cover thumbnails, Instrument Serif titles, mono metadata).
  - **URL paste** — a small form that accepts a book directory URL (e.g. `https://sunprema.github.io/books/books/protobuf/`) and submits it directly. Works for any allowlisted book, including ones not yet in `catalog.json`. Surfaces the import's validation error inline (bad host, missing `book.json`).
- [ ] **23.5** `<.resource_card>` gains a small mono-caps `BOOKBANK` tag when `kind == :bookbank`; clicking a bookbank card routes to the **same** reader route (`/workspaces/:workspace_id/library/:id`). No new reader route.
- [ ] **23.6** `PdfLive.Show` branches on `@resource.kind`. For `:bookbank`, render an `<iframe>` in the center column pointed at `source_url <> concept.file` for the focal concept (`sandbox="allow-scripts"`, lazy). No `PdfCanvasRenderer`, no `file_url`/PDF props. Guard `mount` so bookbank resources skip PDF-specific assigns (annotation lists, collective insights) and load `concepts` + milestones + stamps + chat instead.
- [ ] **23.7** Chapter rail fed from `@resource.concepts` for bookbank (label = concept title, "page" = concept index) instead of waiting for the PDF `outline_loaded` event. `<.chapter_rail>` already accepts `[%{label, page}]`, so this is a data-source swap; `chapter_clicked` sets the focal concept.
- [ ] **23.8** Concept navigation reusing `:focal_page` as focal-concept index — prev/next concept controls in the reader header (mono caps, e.g. `‹ 3 / 17 ›`) and `chapter_clicked` both set `:focal_page`, which swaps the iframe `src`. No `scroll_to_page` push needed (each concept is its own document).
- [ ] **23.9** Concept-level milestones for bookbank: an admin adds a milestone to a **whole concept** via a plain LiveView form (concept picker + label) — no canvas placement, since there's no selection surface. Reuse `Progress.MilestoneMarker` with `page_number` = concept index and `position` = `%{}` (concept-level, no point geometry). `create_milestone` action unchanged; only the placement UI differs.
- [ ] **23.10** Rubber stamps work unchanged (`apply_stamp` against a milestone). For bookbank resources, surface stamp-based progress ("X / N chapters completed") on the library dashboard and reader in place of the annotation-derived `progress_percent` (which reads empty for bookbank). Add a `bookbank_progress` read/calc off stamp counts rather than overloading the annotation calc.
- [ ] **23.11** Right-rail composition for bookbank: the annotation margin column is replaced by a milestones + rubber-stamps panel (concepts list with per-concept "X / N readers" + stamp button) above the existing chat panel + presence "here now". Reuse `<.milestone_panel>` and the chat live_component; the margin-note list and its filter chips are simply not rendered when `kind == :bookbank`.
- [ ] **23.12** Hide PDF-only entry points when `kind == :bookbank`: the `+ Board` / `Board ↗` buttons (Slice 22, PDF thumbnails), reading-journal export link (Slice 19), and the AI "Ask AI" affordance. Reading sprints: keep the timer/start/join (annotation-independent), but the post-sprint "compare notes" reveal is a no-op for bookbank (nothing to compare) — gate it on `kind == :pdf` or on annotation presence.
- [ ] **23.13** CLAUDE.md updates in the same PR: §4.2 gets a carve-out noting the reader renders bookbank resources in a **sandboxed cross-origin iframe** (read-only; no Svelte canvas, no LiveView↔Svelte contract for this path); §3 domain notes `Resource.kind` and that a bookbank Resource is a live-fetched HTML book, not a stored PDF; note explicitly that HTML annotations are a future slice, not this one.
- [ ] **23.14** Tests: `import_bookbank` (member imports, non-member forbidden, host allowlist rejects a non-`sunprema.github.io` URL, `page_count` = concept count); `Bookbank.Source` catalog/book fetch + cache (mock Req); `PdfLive.Show` mount for a `:bookbank` resource (iframe target rendered, chapter rail from concepts, no PDF props, Board/journal/AI entry points absent); concept-level milestone create + stamp + progress count.
- [ ] **23.15** Manual verification: browse the catalog, add a book, open it, page through concepts via the rail, an admin adds a concept milestone, two members stamp it and see the count update live via PubSub, and chat works across two sessions — all against the live `sunprema.github.io/books` content. Confirm the book's own CSS/JS/animations render inside the iframe (CSP permits cross-origin assets from `github.io`).
